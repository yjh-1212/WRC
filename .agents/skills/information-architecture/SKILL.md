...
- For enterprise systems, prefer visible sidebars, nested menus, or sub-menus for persistent submodules
- Supports the primary nav, doesn't duplicate it
- **Do not use Tabs as a substitute for second-level system navigation**

Use a second-level menu + independent route when a child area has one or more of these properties:

- its own business goal or page title
- its own collection/table, filters, search, CRUD, workflow, or permissions
- users need to bookmark/share/deep-link it as a destination
- it is a sibling function users would naturally name in a functional module list
- switching to it changes the user's primary task, not merely the view of the current task

Use in-page Tabs only when all are true:

- the user remains in the same page/task/entity context
- the views share the same primary object or dataset
- the switch is lightweight and does not represent a separate module
- the number of views is small and stable
- hiding the Tabs would not erase an important level of the system's sitemap

**Enterprise default:** if a system has `一级模块 → 多个二级功能`, represent that structure in navigation and URLs such as `/module/submodule`. Do not put all second-level functions into `/module` as a row of Tabs merely to reduce page count.
